  ...
  async message(data) {
    ...
    try {
      if (this.application.api.has(msg)) {
        ...
        if (auth === true && this.isAuthed() === false)
          throw new ServerError(509);
        await this.queue.add(async () => {
          const result = await method(this, args);
          this.send(JSON.stringify({ callId, result }));
          if (liveReload && this.isAuthed() === true) {
            ...
            this.application.channels.sendGroup(
              this.user.login,
              JSON.stringify({ callId: -1, structure })
            );
          }
        });
      }
